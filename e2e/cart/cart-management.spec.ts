...
test.describe('Cart Management', () => {
  ...
  test('should display all cart items', async ({ page }) => {
    ...
    // Verify quantities
    await expect(page.locator('[data-testid="quantity-display"]').nth(0)).toHaveText('2');
    await expect(page.locator('[data-testid="quantity-display"]').nth(1)).toHaveText('1');
    await expect(page.locator('[data-testid="quantity-display"]').nth(2)).toHaveText('3');
  });

  test('should edit item name inline', async ({ page }) => {
    // Click on first item name to enter edit mode
    await page.getByRole('button', { name: 'Nombre del artículo' }).first().click();
  ...
  test('should disable decrease button at minimum quantity', async ({ page }) => {
  ...
  test('should enable generate quote button when cart has items', async ({ page }) => {
    const generateQuoteButton = page.getByRole('button', { name: 'Generar cotización' });
