...
test.describe('Empty Cart State', () => {
  ...
  test('should display empty cart message', async ({ page }) => {
    ...
    await expect(page.getByText('Agrega configuraciones de ventanas desde el catálogo')).toBeVisible();
  });
  ...
  test('should not show generate quote button when empty', async ({ page }) => {
    ...
    const generateButton = page.getByRole('button', { name: 'Generar cotización' });
  ...
  test('should have proper ARIA structure for empty state', async ({ page }) => {
    // Verify heading hierarchy
    const heading = page.getByRole('heading', { name: 'Tu carrito está vacío' });
