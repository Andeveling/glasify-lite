...
test.describe('Quotes Table - My Quotes Page', () => {
  ...
  test('should sort quotes by date', async ({ page }) => {
    ...
    // Get all date cells
    const dates = await page.locator('tbody tr td:nth-child(5) p').allTextContents();
  ...
  test('should display quote actions menu', async ({ page }) => {
    ...
    await expect(page.getByRole('menuitem', { name: /descargar pdf/i })).toBeVisible();
  });
  ...
  test('should display empty state when no quotes found', async ({ page }) => {
    ...
    const emptyMessage = page.locator('text=/no se encontraron|no hay cotizaciones/i');
