...
test.describe("Quote Status Clarity (US1)", () => {
  ...
  test('should display "En edición" for draft quotes instead of "Borrador"', async ({
    page,
  }) => {
    // Verify draft status badge shows correct label
    const draftBadge = page
      .getByTestId("quote-status-badge")
      .filter({ hasText: "En edición" });
  ...
  test("should display tooltip with explanatory text on hover", async ({
    page,
  }) => {
  ...
  test("should show different labels for each status type", async ({
    page,
  }) => {
  ...
  test("should navigate to quote detail when clicking CTA", async ({
    page,
  }) => {
  ...
  test("should display status badge in quote detail view", async ({ page }) => {
    ...
    const viewButton = firstQuote.getByRole("button", {
      name: /ver detalles|continuar editando/i,
    });
  ...
  test("should show tooltip in quote detail view on hover", async ({
    page,
  }) => {
  ...
  test("should use color-coded badges for different statuses", async ({
    page,
  }) => {
  ...
  test("should maintain status clarity with expired quotes", async ({
    page,
  }) => {
  ...
  test("should show consistent status labels across list and detail views", async ({
    page,
  }) => {
    // Get status label from list view
    const listStatusBadge = page.getByTestId("quote-status-badge").first();
    const listStatusText = await listStatusBadge
      .getByTestId("status-label")
      .textContent();
    ...
    const detailStatusText = await detailStatusBadge
      .getByTestId("status-label")
      .textContent();
...
test.describe("Quote Status Configuration Edge Cases", () => {
  test("should handle missing or unknown status gracefully", async ({
    page,
  }) => {
  ...
  test("should render without JavaScript (progressive enhancement)", async ({
    page,
  }) => {
    // Disable JavaScript
