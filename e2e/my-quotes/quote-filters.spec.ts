...
test.describe("Quote Filtering and Search", () => {
  ...
  test.describe("Status Filter", () => {
    ...
    test('should show all quotes when "Todas" is selected', async ({
      page,
    }) => {
  ...
  test.describe("Sort Functionality", () => {
    test("should sort quotes by newest first (default)", async ({ page }) => {
      ...
      const quotes = await page
        ...
        .all();
      if (quotes.length >= 2) {
        const firstDate = await quotes[0]
          ?.locator('[data-testid="quote-date"]')
          .textContent();
        const secondDate = await quotes[1]
          ?.locator('[data-testid="quote-date"]')
          .textContent();
  ...
  test.describe("Combined Filters", () => {
    ...
    test("should apply all three filters (status + search + sort)", async ({
      page,
    }) => {
      // Status
      await page.click('[data-testid="status-filter"]');
    ...
    test("should preserve page param when applying filters", async ({
      page,
    }) => {
  ...
  test.describe("Empty Results State", () => {
    ...
    test("should show clear filters suggestion in empty state", async ({
      page,
    }) => {
      ...
      const clearButton = emptyState.locator(
  ...
  test.describe("Accessibility", () => {
    ...
    test("should have proper ARIA labels", async ({ page }) => {
      ...
      await expect(statusFilter).toHaveAttribute(
        "aria-label",
        /filtrar por estado/i
      );

      const searchInput = page.locator('[data-testid="search-input"]');
      await expect(searchInput).toHaveAttribute(
        "aria-label",
        /buscar cotizaciones/i
      );

      const sortSelect = page.locator('[data-testid="sort-select"]');
      await expect(sortSelect).toHaveAttribute(
        "aria-label",
        /ordenar cotizaciones/i
      );
    });
  });
