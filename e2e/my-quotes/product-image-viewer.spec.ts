...
test.describe('Product Image Viewer & Lightbox', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.getByRole('link', { name: /cotización/i }).first().click();
    await page.waitForLoadState('networkidle');
  });

  test.describe('Product Thumbnail Display', () => {
    test('should display product thumbnail images for each quote item', async ({ page }) => {
    ...
    test('should display images in responsive grid (3-4 columns)', async ({ page }) => {
    ...
    test('should show SVG diagram fallback when image unavailable', async ({ page }) => {
      // Find item with missing image (should show WindowDiagram)
      const svgFallback = page.locator('[data-testid="quote-item-image"]').filter({
    ...
    test('should apply lazy loading to images below the fold', async ({ page }) => {
      const images = page.locator('[data-testid="quote-item-image"] img').filter({
    ...
    test('should show first 3 items as preview on quote list', async ({ page }) => {
      // Go back to quote list
      await page.goBack();
      await page.waitForLoadState('networkidle');
      
      // Verify preview thumbnails in quote list item
      const previewThumbnails = page.locator('[data-testid="quote-item-preview"]');
      
      if (await previewThumbnails.count() > 0) {
        await expect(previewThumbnails.first()).toBeVisible();
        
        // Should show max 3 thumbnails
        const thumbnails = previewThumbnails.first().locator('[data-testid="quote-item-image"]');
  ...
  test.describe('Image Lightbox', () => {
    test('should open lightbox when clicking product thumbnail', async ({ page }) => {
      // Click first product thumbnail
      const thumbnail = page.locator('[data-testid="quote-item-image"]').first();
    ...
    test('should display product specifications in lightbox', async ({ page }) => {
      // Open lightbox
    ...
    test('should close lightbox when clicking overlay background', async ({ page }) => {
      // Open lightbox
      await page.locator('[data-testid="quote-item-image"]').first().click();
    ...
    test('should show SVG diagram in lightbox for items without images', async ({ page }) => {
  ...
  test.describe('Image Optimization & Performance', () => {
    test('should load images progressively (lazy loading)', async ({ page }) => {
    ...
    test('should complete image load in under 2 seconds', async ({ page }) => {
      ...
      await page.locator('[data-testid="quote-item-image"] img').first().waitFor({ state: 'visible' });
  ...
  test.describe('Accessibility', () => {
    ...
    test('should announce image changes to screen readers', async ({ page }) => {
      // Open lightbox
      await page.locator('[data-testid="quote-item-image"]').first().click();
  ...
  test.describe('Edge Cases', () => {
    ...
    test('should handle very long product names in lightbox', async ({ page }) => {
      // Open lightbox
      await page.locator('[data-testid="quote-item-image"]').first().click();
      
      const lightbox = page.locator('[data-testid="image-viewer-dialog"]');
      const productName = lightbox.getByText(/modelo:/i).locator('..').getByText(/.+/);
