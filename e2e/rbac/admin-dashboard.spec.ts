...
test.describe('Admin Dashboard Flow', () => {
  test.describe('Admin User Access', () => {
    ...
    test('should display admin navigation menu', async ({ page }) => {
      ...
      await expect(page.getByRole('link', { name: /cotizaciones/i })).toBeVisible();
    ...
    test('should display "Create Model" button (admin-only action)', async ({ page }) => {
      // Login as admin
      await page.goto('/signin');
      await page.getByLabel(/email/i).fill(ADMIN_USER.email);
      await page.getByLabel(/contraseña/i).fill(ADMIN_USER.password);
  ...
  test.describe('Non-Admin User Restrictions', () => {
    test('should redirect non-admin from /dashboard to /my-quotes', async ({ page }) => {
    ...
    test('should block direct access to /dashboard for non-admin', async ({ page }) => {
      // Login as regular user
      await page.goto('/signin');
      await page.getByLabel(/email/i).fill(NON_ADMIN_USER.email);
      await page.getByLabel(/contraseña/i).fill(NON_ADMIN_USER.password);
    ...
    test('should block access to /dashboard/models for non-admin', async ({ page }) => {
      // Login as regular user
      await page.goto('/signin');
      await page.getByLabel(/email/i).fill(NON_ADMIN_USER.email);
      await page.getByLabel(/contraseña/i).fill(NON_ADMIN_USER.password);
    ...
    test('should block access to /dashboard/quotes for non-admin', async ({ page }) => {
      // Login as regular user
      await page.goto('/signin');
      await page.getByLabel(/email/i).fill(NON_ADMIN_USER.email);
      await page.getByLabel(/contraseña/i).fill(NON_ADMIN_USER.password);
    ...
    test('should not display admin navigation for non-admin', async ({ page }) => {
      // Login as regular user
      await page.goto('/signin');
      await page.getByLabel(/email/i).fill(NON_ADMIN_USER.email);
      await page.getByLabel(/contraseña/i).fill(NON_ADMIN_USER.password);
  ...
  test.describe('Unauthenticated Access', () => {
    test('should redirect unauthenticated user to /signin with callbackUrl', async ({ page }) => {
    ...
    test('should redirect to dashboard after signin with callbackUrl', async ({ page }) => {
