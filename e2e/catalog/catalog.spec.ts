...
test.describe("Catalog Page", () => {
  ...
  test.describe("Initial Page Load", () => {
    test("should display catalog header and description", async ({ page }) => {
      ...
      await expect(
        page.getByText("Explore nuestra selección de productos de vidrio")
      ).toBeVisible();
    });
    ...
    test("should display all filter controls", async ({ page }) => {
      ...
      await expect(
        page.getByRole("combobox").filter({ hasText: "Todos" })
      ).toBeVisible();
  ...
  test.describe("Search Functionality", () => {
    test("should filter products by search query", async ({ page }) => {
      ...
      await expect(page.getByText("Ventana Panorama 2024")).toBeVisible();
      await expect(
        page.getByText("Ventana Termoacústica Elite")
      ).not.toBeVisible();
    });
  ...
  test.describe("Profile Supplier Filter", () => {
    test("should filter products by profile supplier", async ({ page }) => {
      ...
      await expect(
        page.getByRole("option", { name: "Cristales Modernos" })
      ).toBeVisible();
      await expect(
        page.getByRole("option", { name: "Ventanas Andinas" })
      ).toBeVisible();
      ...
      await expect(
        page
          .locator(".max-w-\\[200px\\]")
          .filter({ hasText: "Ventanas Andinas" })
      ).toBeVisible();
    ...
    test("should clear profile supplier filter using badge button", async ({
      page,
    }) => {
      ...
      await page
        .getByRole("button", { name: "Quitar filtro de Ventanas Andinas" })
        .click();
  ...
  test.describe("Clear All Filters", () => {
    test("should clear all search parameters with clear all button", async ({
      page,
    }) => {
      ...
      await page
        .getByRole("button", { name: "Limpiar todos los parámetros" })
        .click();
  ...
  test.describe("Product Navigation", () => {
    test("should navigate to product detail page when clicking a product card", async ({
      page,
    }) => {
      ...
      await expect(
        page.getByRole("heading", { name: "Ventana Corrediza Premium" })
      ).toBeVisible();
    ...
    test("should maintain filter state when navigating back from product detail", async ({
      page,
    }) => {
      // Aplicar filtro
      const searchInput = page.getByPlaceholder("Buscar productos...");
      ...
      await expect(page.getByPlaceholder("Buscar productos...")).toHaveValue(
        "Panorama"
      );
    });
  });
  ...
  test.describe("Model Cards Content", () => {
    test("should display correct product information in cards", async ({
      page,
    }) => {
      // Verificar información del primer producto usando selectores más específicos
      await expect(
        page.getByRole("heading", { name: "Ventana Panorama 2024" })
      ).toBeVisible();
      ...
      await expect(
        page.getByRole("heading", { name: "Ventana Termoacústica Elite" })
      ).toBeVisible();
    ...
    test("should have proper accessibility attributes on product cards", async ({
      page,
    }) => {
      // Verificar que los enlaces tienen texto descriptivo
      await expect(
        page.getByRole("link", { name: PANORAMA_PRODUCT_LINK })
      ).toBeVisible();
      await expect(
        page.getByRole("link", { name: TERM_ACOUSTIC_PRODUCT_LINK })
      ).toBeVisible();
  ...
  test.describe("Performance and Loading States", () => {
    test("should show loading skeleton while content loads", async ({
      page,
    }) => {
