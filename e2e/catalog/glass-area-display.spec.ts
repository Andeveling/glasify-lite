...
test.describe("Catalog Model Form - Glass Area Display", () => {
  ...
  test("should display calculated glass price instead of price per m² when dimensions are set", async ({
    page,
  }) => {
  ...
  test("should show area in price breakdown popover", async ({ page }) => {
    ...
    const glassBreakdownItem = page
      .locator("text=/Vidrio.*\\(1\\.50 m²\\)/i")
      .first();
