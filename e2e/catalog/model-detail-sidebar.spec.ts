...
test.describe("Catalog Model Detail - Sidebar Components", () => {
  test.describe("ProfileSupplierCard - With Supplier", () => {
    ...
    test("should display profile supplier card with name and material badge", async ({
      page,
    }) => {
      // Verify card heading
      const supplierCard = page
        .locator("text=Proveedor de Perfiles")
        .locator("..");
      await expect(supplierCard).toBeVisible();

      // Verify supplier name is displayed (any non-empty text)
      const supplierName = supplierCard
        .locator("h4, p")
        .filter({ hasNotText: "Proveedor de Perfiles" })
        .first();
      ...
      const materialBadge = supplierCard
        .locator('[role="badge"], .badge, [class*="badge"]')
        .first();
      ...
      expect([
        ...
        "MIXED",
        "MIXTO",
      ]).toContainEqual(badgeText?.trim().toUpperCase());
    });
    ...
    test("should show PVC-specific benefits for PVC models", async ({
      page,
    }) => {
      ...
      if (badgeText?.toUpperCase().includes("PVC")) {
        ...
        const hasKeyword = pvcKeywords.some((keyword) =>
          benefitsText?.toLowerCase().includes(keyword)
        );
        expect(hasKeyword).toBe(true);
      }
    });

    test("should show Aluminum-specific benefits for Aluminum models", async ({
      page,
    }) => {
      const supplierCard = page
        .locator("text=Proveedor de Perfiles")
      ...
      if (badgeText?.toUpperCase().includes("ALUM")) {
        ...
        const aluminumKeywords = [
          "resistencia estructural",
          "perfiles delgados",
          "estética moderna",
          "durabilidad",
        ];
        const hasKeyword = aluminumKeywords.some((keyword) =>
          benefitsText?.toLowerCase().includes(keyword)
        );
        expect(hasKeyword).toBe(true);
      }
  ...
  test.describe("ProfileSupplierCard - Without Supplier (NULL handling)", () => {
    ...
    test.skip('should display "Proveedor no especificado" when supplier is NULL', async ({
      page,
    }) => {
  ...
  test.describe("ModelSpecifications Card", () => {
    ...
    test("should display technical specifications with performance ratings", async ({
      page,
    }) => {
      const specsCard = page
        .locator("text=Especificaciones Técnicas")
        .locator("..");
      ...
      const thermalRating = specsCard
        .locator("text=Aislamiento Térmico")
        .locator("..");
      await expect(thermalRating).toBeVisible();
      await expect(thermalRating.locator("text=★")).toBeVisible();

      const acousticRating = specsCard
        .locator("text=Aislamiento Acústico")
        .locator("..");
      await expect(acousticRating).toBeVisible();

      const structuralRating = specsCard
        .locator("text=Resistencia Estructural")
        .locator("..");
      await expect(structuralRating).toBeVisible();
    });

    test("should show dimensional constraints in specifications", async ({
      page,
    }) => {
      const specsCard = page
        .locator("text=Especificaciones Técnicas")
      ...
      await expect(
        specsCard.locator("text=Capacidades Dimensionales")
      ).toBeVisible();
      ...
      const dimensionsSection = specsCard
        .locator("text=Capacidades Dimensionales")
        .locator("..");
    ...
    test("should display 1-5 stars for performance ratings", async ({
      page,
    }) => {
      const specsCard = page
        .locator("text=Especificaciones Técnicas")
      ...
      const starContainers = specsCard.locator(
      ...
      for (let i = 0; i < count; i++) {
        const ariaLabel = await starContainers
          .nth(i)
    ...
    test("should NOT display specifications when supplier is NULL", async ({
      page,
    }) => {
  ...
  test.describe("ModelDimensions - Dimensional Guidelines (User Story 4)", () => {
    ...
    test("should display dimensional constraints card with clear min/max values", async ({
      page,
    }) => {
      // Verify card heading
      const dimensionsCard = page
        .locator("text=Dimensiones Permitidas")
        .locator("..");
    ...
    test('should display dimension values in millimeters with "mm" suffix', async ({
      page,
    }) => {
      const dimensionsCard = page
        .locator("text=Dimensiones Permitidas")
    ...
    test("should show valid dimensional ranges (min < max)", async ({
      page,
    }) => {
      const dimensionsCard = page
        .locator("text=Dimensiones Permitidas")
      ...
      const minWidthRow = dimensionsCard
        .locator("text=Ancho mínimo:")
        .locator("..");
      const minWidthValue = await minWidthRow
        .locator(".font-medium")
        .textContent();
      const minWidth = Number.parseInt(
        minWidthValue?.replace("mm", "") ?? "0",
        10
      );

      const maxWidthRow = dimensionsCard
        .locator("text=Ancho máximo:")
        .locator("..");
      const maxWidthValue = await maxWidthRow
        .locator(".font-medium")
        .textContent();
      const maxWidth = Number.parseInt(
        maxWidthValue?.replace("mm", "") ?? "0",
        10
      );

      // Extract min/max height values
      const minHeightRow = dimensionsCard
        .locator("text=Alto mínimo:")
        .locator("..");
      const minHeightValue = await minHeightRow
        .locator(".font-medium")
        .textContent();
      const minHeight = Number.parseInt(
        minHeightValue?.replace("mm", "") ?? "0",
        10
      );

      const maxHeightRow = dimensionsCard
        .locator("text=Alto máximo:")
        .locator("..");
      const maxHeightValue = await maxHeightRow
        .locator(".font-medium")
        .textContent();
      const maxHeight = Number.parseInt(
        maxHeightValue?.replace("mm", "") ?? "0",
        10
      );
    ...
    test("should display unusual capacities prominently (e.g., 6700mm sliding doors)", async ({
      page,
    }) => {
      const dimensionsCard = page
        .locator("text=Dimensiones Permitidas")
    ...
    test("should be easy to reference while filling quote form", async ({
      page,
    }) => {
    ...
    test("should display dimensions card on mobile viewport for easy reference", async ({
      page,
    }) => {
  ...
  test.describe("Responsive Layout", () => {
    test("should display cards vertically on mobile viewport", async ({
      page,
    }) => {
      // Set mobile viewport
      await page.setViewportSize({ height: 667, width: 375 });
      ...
      if (specsBox && supplierBox) {
        ...
        expect(
          Math.abs((specsBox.y ?? 0) - (supplierBox.y ?? 0))
        ).toBeGreaterThan(50);
      }
    });

    test("should maintain card readability on tablet viewport", async ({
      page,
    }) => {
