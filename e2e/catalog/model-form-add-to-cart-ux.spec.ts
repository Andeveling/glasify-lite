...
test.describe('Catalog Model Form - Add to Cart UX', () => {
  ...
  test('should show success actions after adding to cart', async ({ page }) => {
    ...
    // Submit form
    const submitButton = page.getByRole('button', { name: ADD_TO_CART_BUTTON_REGEX });
    ...
    await expect(page.getByRole('button', { name: CONFIGURE_ANOTHER_BUTTON_REGEX })).toBeVisible();
    await expect(page.getByRole('button', { name: CATALOG_BUTTON_REGEX })).toBeVisible();
    await expect(page.getByRole('button', { name: CART_BUTTON_REGEX })).toBeVisible();
  });
  ...
  test('should reset form when configuring another item', async ({ page }) => {
    ...
    const configureAnotherButton = page.getByRole('button', { name: CONFIGURE_ANOTHER_BUTTON_REGEX });
    ...
    await expect(page.locator('text=¡Agregado exitosamente!')).not.toBeVisible();
  ...
  test('should navigate to catalog when clicking catalog button', async ({ page }) => {
    ...
    const catalogButton = page.getByRole('button', { name: CATALOG_BUTTON_REGEX });
  ...
  test('should navigate to cart when clicking cart button', async ({ page }) => {
    // Fill and submit form
    const widthInput = page.locator('input[name="width"]');
    const heightInput = page.locator('input[name="height"]');
  ...
  test('should allow adding multiple items by using configure another', async ({ page }) => {
    // Add first item
    const widthInput = page.locator('input[name="width"]');
    const heightInput = page.locator('input[name="height"]');
