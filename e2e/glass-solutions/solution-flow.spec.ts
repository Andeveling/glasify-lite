...
test.describe("Glass Solutions - User Journey", () => {
  ...
  test("should display catalog with models", async ({ page }) => {
    ...
    const manufacturerFilter = page
      .locator("text=/Guardian|AGC|Saint-Gobain|VEKA/i")
      .first();
    await expect(manufacturerFilter).toBeVisible();
  });

  test("should open model configuration when clicking a model", async ({
    page,
  }) => {
  ...
  test("should show solution selector as first step", async ({ page }) => {
    ...
    const solutionSection = page
      .locator("text=/Selecciona.*soluci\u00f3n|Tipo de vidrio/i")
      .first();
    ...
    const thermalSolution = page
      .locator("text=/T\u00e9rmico|Thermal|Aislamiento/i")
      .first();
  ...
  test("should filter glass types when selecting a solution", async ({
    page,
  }) => {
    // Navigate to a model
    await page.locator('[data-testid="model-card"]').first().click();
    ...
    const anySolutionButton = page
      .locator('[data-testid="solution-button"]')
      .first();

    const solutionToClick =
      (await securityButton.count()) > 0
        ? securityButton.first()
        : anySolutionButton;
    ...
    const glassTypeSection = page
      .locator("text=/Selecciona.*tipo|vidrio disponible/i")
      .first();
    const glassTypeVisible = await glassTypeSection
      .isVisible()
      .catch(() => false);
  ...
  test("should display performance ratings on glass cards", async ({
    page,
  }) => {
    // Navigate to a model
    await page.locator('[data-testid="model-card"]').first().click();
    ...
    if (cardCount > 0) {
      ...
      const hasStars =
        (await firstCard.locator('svg[class*="star"]').count()) > 0;
      const hasBadges =
        (await firstCard.locator('[class*="badge"]').count()) > 0;
      const hasRatingIcons =
        (await firstCard.locator("svg").count()) >= MIN_RATING_ICONS;
  ...
  test("should allow selecting a glass type", async ({ page }) => {
    ...
    const glassTypeButton = page
      .locator('[data-testid="glass-type-button"]')
      .first();
    ...
    if (glassTypeCount > 0) {
      ...
      const isChecked =
        (await glassTypeButton.locator('[aria-checked="true"]').count()) > 0;
      const isSelected =
        (await glassTypeButton.locator('[data-selected="true"]').count()) > 0;
      const hasCheckmark =
        (await glassTypeButton.locator('svg[class*="check"]').count()) > 0;
  ...
  test("should show price information", async ({ page }) => {
    ...
    const pricePatternVisible = await pricePattern
      .isVisible()
      .catch(() => false);
  ...
  test("should support keyboard navigation", async ({ page }) => {
    ...
    const isSolutionButton =
      (await focusedElement.getAttribute("data-testid")) === "solution-button";
    ...
    if (isSolutionButton) {
      ...
      const glassTypeSection = page
        .locator('[data-testid="glass-type-section"]')
        .first();
