...
test.describe("Quote History", () => {
  ...
  test("should display quotes list for authenticated user", async ({
    page,
  }) => {
    // Verify page title
    ...
    // Verify description
    await expect(
      page.getByText("Gestiona y revisa todas tus cotizaciones")
    ).toBeVisible();
  });
  ...
  test("should display quote items with correct information", async ({
    page,
  }) => {
    ...
    const statusBadge = firstQuote.locator(
      '[data-testid="quote-status-badge"]'
    );
    if (await statusBadge.isVisible().catch(() => false)) {
      const statusText = await statusBadge.textContent();
      expect(["Borrador", "Enviada", "Cancelada"]).toContain(
        statusText?.trim()
      );
    }
    ...
    await expect(
      firstQuote.getByRole("link", { name: /ver detalles/i })
    ).toBeVisible();
  });

  test('should navigate to quote detail page when clicking "Ver detalles"', async ({
    page,
  }) => {
    // Check if quotes exist
    const firstQuote = page.locator('[data-testid="quote-list-item"]').first();
    ...
    await expect(
      page.getByRole("link", { name: /volver a cotizaciones/i })
    ).toBeVisible();
  ...
  test("should mark expired quotes with visual indicator", async ({ page }) => {
    ...
    const parentCard = expiredBadge.locator(
      'xpath=ancestor::*[@data-testid="quote-list-item"]'
    );
    const hasOpacity = await parentCard.evaluate(
      (el) => window.getComputedStyle(el).opacity === "0.6"
    );
