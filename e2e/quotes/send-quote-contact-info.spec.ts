...
test.describe("Send Quote - Contact Info Pre-fill (US2)", () => {
  ...
  test("should show empty modal if contactPhone is missing", async ({
    page,
  }) => {
  ...
  test("should allow user to edit pre-filled contactPhone", async ({
    page,
  }) => {
    await page.goto(`/quotes/${quoteWithPhoneId}`);
  ...
  test("should show validation error for invalid phone edit", async ({
    page,
  }) => {
    await page.goto(`/quotes/${quoteWithPhoneId}`);
  ...
  test("should allow user to cancel modal and keep quote as draft", async ({
    page,
  }) => {
    await page.goto(`/quotes/${quoteWithPhoneId}`);
