...
test.describe("Expired Quotes Display", () => {
  ...
  test("should apply muted styling to expired quotes", async ({ page }) => {
    ...
    if (itemCount > 0) {
      ...
      for (let i = 0; i < itemCount; i++) {
        ...
        const hasExpiredBadge =
          (await item.getByTestId("expired-badge").count()) > 0;
  ...
  test("should show expired quotes when includeExpired=true", async ({
    page,
  }) => {
  ...
  test("should display expired badge in quote detail view", async ({
    page,
  }) => {
    await page.goto("/quotes?includeExpired=true");
    ...
    if (hasExpiredQuote) {
      ...
      const detailExpiredBadge = page
        .locator("text=/expirada/i")
        .filter({ hasText: /expirada/i });
      await expect(detailExpiredBadge.first()).toBeVisible();
    }
  });

  test("should calculate expiration correctly based on validUntil date", async ({
    page,
  }) => {
    await page.goto("/quotes?includeExpired=true");
    ...
    if (itemCount > 0) {
      for (let i = 0; i < Math.min(itemCount, 3); i++) {
        ...
        if (validUntilText) {
          ...
          const dateMatch = validUntilText.match(
            /\d{1,2}[/-]\d{1,2}[/-]\d{2,4}/
          );
  ...
  test("should support filtering expired quotes by status", async ({
    page,
  }) => {
  ...
  test("should maintain expired filter when navigating pages", async ({
    page,
  }) => {
  ...
  test("should display expired quotes with correct ARIA attributes", async ({
    page,
  }) => {
    await page.goto("/quotes?includeExpired=true");
  ...
  test('should show "Sin límite" for quotes without validUntil date', async ({
    page,
  }) => {
    await page.goto("/quotes");
