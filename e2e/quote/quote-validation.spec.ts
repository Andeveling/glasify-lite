...
test.describe('Empty Cart Prevention (US4-T063)', () => {
  ...
  test('should redirect to catalog if accessing /quote/new with empty cart', async ({ page }) => {
  ...
  test('should show empty cart message if cart becomes empty', async ({ page }) => {
...
test.describe('Address Validation Errors (US4-T063)', () => {
  ...
  test.skip('should show validation errors for empty required fields', async ({ page }) => {
    ...
    const validationErrors = page.locator(`text=${REQUIRED_FIELD_PATTERN.source}`);
    await expect(validationErrors.first()).toBeVisible({ timeout: VALIDATION_TIMEOUT_MS });
  ...
  // biome-ignore lint/suspicious/noSkippedTests: requires authentication, pending auth mock setup
  test.skip('should show validation error when street field exceeds max length', async ({ page }) => {
    // NOTE: This test requires authentication to access /quote/new
    // Skip until auth mock is implemented
    ...
    const validationError = page.locator(`text=${FIELD_TOO_LONG_PATTERN.source}`);
    await expect(validationError).toBeVisible({ timeout: VALIDATION_TIMEOUT_MS });
  });

  // biome-ignore lint/suspicious/noSkippedTests: requires authentication, pending auth mock setup
  test.skip('should show validation error when city field exceeds max length', async ({ page }) => {
    // NOTE: This test requires authentication to access /quote/new
    // Skip until auth mock is implemented
    ...
    // Wait for form to load
    await expect(page.locator('input[name="projectCity"]')).toBeVisible({ timeout: FORM_TIMEOUT_MS });
  ...
  // biome-ignore lint/suspicious/noSkippedTests: requires authentication, pending auth mock setup
  test.skip('should validate all required fields independently', async ({ page }) => {
    // NOTE: This test requires authentication to access /quote/new
    // Skip until auth mock is implemented
