...
test.describe('Quote Navigation from Cart', () => {
  ...
  test('should navigate to quote generation page from cart without redirect loop', async ({ page }) => {
    // Step 1: Navigate to catalog
    await page.getByRole('link', { name: /todos los productos|catálogo/i }).click();
    ...
    await expect(page.getByRole('heading', { name: /carrito de presupuesto/i })).toBeVisible();
    ...
    const generateQuoteButton = page.getByRole('button', { name: /generar cotización/i });
    ...
    if (isSigninPage) {
      ...
      test.skip(!isSigninPage, 'User must be authenticated to continue this test');
    }
  });

  test('should show quote form after authentication without redirect to catalog', async ({ page, context }) => {
    ...
    const submitButton = page.getByRole('button', { name: /generar cotización/i });
  ...
  test('should redirect to catalog if cart is actually empty', async ({ page }) => {
    ...
    await expect(page.getByText(/carrito vacío/i)).toBeVisible({ timeout: 2000 });
  });
});
