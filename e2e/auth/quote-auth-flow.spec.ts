...
test.describe('Quote Authentication Flow', () => {
  ...
  test('should redirect unauthenticated user to sign-in when clicking "Generate Quote"', async ({ page }) => {
    ...
    await expect(page.locator(`text=${ADDED_TO_CART_REGEX.source}`)).toBeVisible({ timeout: 5000 });
    ...
    const generateQuoteButton = page.locator('button:has-text("Generar cotización")');
  ...
  test('should preserve cart items during auth redirect', async ({ page }) => {
    ...
    const cartItemsBefore = await page.locator('[data-testid="cart-item"]').count();
    ...
    const cartItemsAfter = await page.locator('[data-testid="cart-item"]').count();
    expect(cartItemsAfter).toBe(2);
  });

  test('should show authentication hint in CartSummary when not authenticated', async ({ page }) => {
    ...
    await expect(page.locator(`text=${AUTH_REQUIRED_REGEX.source}`)).toBeVisible();
  });

  test('should protect /quotes route from unauthenticated access', async ({ page }) => {
  ...
  test('should protect /quote/new route from unauthenticated access', async ({ page }) => {
