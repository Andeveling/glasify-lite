...
const PASSWORD_LENGTH_ERROR_REGEX =
...
test.describe("Authentication E2E Tests", () => {
  ...
  test.describe("Sign In Page UI", () => {
    test("should render signin page correctly", async ({ page }) => {
      ...
      // Check main heading
      await expect(
        page.getByRole("heading", { name: SIGNIN_HEADING_REGEX })
      ).toBeVisible();
      ...
      // Check buttons
      await expect(
        page.getByRole("button", { name: SIGNIN_HEADING_REGEX })
      ).toBeVisible();
      await expect(
        page.getByRole("button", { name: GOOGLE_BUTTON_REGEX })
      ).toBeVisible();
  ...
  test.describe("Google OAuth Integration", () => {
    test("should have Google sign in button with proper styling", async ({
      page,
    }) => {
      const googleButton = page.getByRole("button", {
        name: GOOGLE_BUTTON_REGEX,
      });
  ...
  test.describe("Protected Route Redirection", () => {
    ...
    test("should redirect to intended page after successful login", async ({
      page,
    }) => {
  ...
  test.describe("Performance", () => {
    ...
    test("should handle form interactions smoothly", async ({ page }) => {
      ...
      await expect(page.getByLabel(EMAIL_LABEL_REGEX)).toHaveValue(
        "test@example.com"
      );
      await expect(page.getByLabel(PASSWORD_LABEL_REGEX)).toHaveValue(
        "password123"
      );
    });
  });
