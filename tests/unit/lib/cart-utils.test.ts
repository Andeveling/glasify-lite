...
describe("calculateCartTotal", () => {
  ...
  it("should handle decimal prices correctly", () => {
    const items = [
      createMockCartItem({ subtotal: 10_000.5 }),
      createMockCartItem({ subtotal: 20_000.25 }),
    ];
...
describe("validateQuantity", () => {
  ...
  it("should accept minimum quantity", () => {
    expect(validateQuantity(CART_CONSTANTS.MIN_QUANTITY)).toEqual({
      valid: true,
    });
  });

  it("should accept maximum quantity", () => {
    expect(validateQuantity(CART_CONSTANTS.MAX_QUANTITY)).toEqual({
      valid: true,
    });
  });
...
describe("validateCartLimit", () => {
  it("should accept cart below limit", () => {
    ...
    expect(validateCartLimit(CART_CONSTANTS.MAX_ITEMS - 1)).toEqual({
      valid: true,
    });
  });
...
describe("validateCartItem", () => {
  ...
  it("should accept item with populated selectedServiceIds", () => {
    const item = createMockCartItem({
      additionalServiceIds: ["service-1", "service-2"],
    });
    expect(validateCartItem(item)).toEqual({ valid: true });
  });
