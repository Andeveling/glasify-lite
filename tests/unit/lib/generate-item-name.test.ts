...
describe('extractModelPrefix', () => {
  ...
  it('should limit prefix to 10 characters', () => {
    expect(extractModelPrefix('VeryLongModelNameThatExceeds')).toBe('VERYLONGMO');
...
describe('findNextSequence', () => {
  ...
  it('should find next sequence for existing prefix', () => {
    const items = [createMockCartItem('VEKA-001', 'VEKA Premium'), createMockCartItem('VEKA-002', 'VEKA Premium')];
...
describe('generateItemName', () => {
  ...
  it('should not exceed maximum name length', () => {
    const name = generateItemName('VeryLongModelNameThatExceedsMaximumLength', []);
...
describe('validateItemName', () => {
  it('should accept valid names', () => {
    ...
    expect(validateItemName('Mi Ventana Personalizada', [])).toEqual({ valid: true });
  });
