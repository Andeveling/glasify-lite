...
describe('QuoteStatusBadge Component', () => {
  ...
  describe('Tooltip Behavior', () => {
    ...
    it('should show different tooltip content for each status', async () => {
      expect(async () => {
        ...
        await user.hover(badge);
        expect(await screen.findByRole('tooltip')).toHaveTextContent(/enviada al cliente/i);

        rerender(<QuoteStatusBadge status="canceled" />);
        badge = screen.getByText('Cancelada');
        await user.hover(badge);
        expect(await screen.findByRole('tooltip')).toHaveTextContent(/cancelada/i);
      }).rejects.toThrow();
    });
  });
  ...
  describe('Accessibility', () => {
    ...
    it('icon should be aria-hidden (decorative)', () => {
      expect(() => {
        render(<QuoteStatusBadge status="draft" />);
        const icon = screen.getByText('En edición').closest('span')?.querySelector('svg');
