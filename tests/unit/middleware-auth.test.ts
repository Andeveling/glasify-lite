...
describe("Middleware Authorization Logic", () => {
  ...
  describe("Redirect URL Validation", () => {
    ...
    it("unauthenticated users redirected to signin with callbackUrl", () => {
      expect(expectedRedirects.unauthenticated["/dashboard"]).toBe(
        "/signin?callbackUrl=%2Fdashboard"
      );
      expect(expectedRedirects.unauthenticated["/quotes"]).toBe(
        "/signin?callbackUrl=%2Fquotes"
      );
      expect(expectedRedirects.unauthenticated["/my-quotes"]).toBe(
        "/signin?callbackUrl=%2Fmy-quotes"
      );
    });
  });
