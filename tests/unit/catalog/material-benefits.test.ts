...
describe("Material Benefits Utilities", () => {
  describe("MATERIAL_BENEFITS", () => {
    ...
    it("should provide Spanish language benefits", () => {
      ...
      expect(
        pvcBenefits.some((benefit: string) => benefit.includes("aislamiento"))
      ).toBe(true);
      expect(
        pvcBenefits.every((benefit: string) => typeof benefit === "string")
      ).toBe(true);
    });
    ...
    it("should have distinct benefits per material type", () => {
      ...
      expect(
        pvcBenefits.some((b: string) => b.toLowerCase().includes("térmico"))
      ).toBe(true);

      // Aluminum should emphasize structural strength
      expect(
        aluminumBenefits.some((b: string) =>
          b.toLowerCase().includes("resistencia")
        )
      ).toBe(true);
    });
  ...
  describe("formatPerformanceRating", () => {
    ...
    it("should return Spanish labels for all levels", () => {
      ...
      for (const level of levels) {
        ...
        expect(
          result.label.match(/[áéíóúñÁÉÍÓÚÑ]|Excelente|Bueno|Estándar/)
        ).toBeTruthy();
      }
    });
  ...
  describe("Integration - Material to User Display", () => {
    ...
    it("should provide complete data flow for ALUMINUM material", () => {
      ...
      expect(benefits.some((b: string) => b.includes("resistencia"))).toBe(
        true
      );
