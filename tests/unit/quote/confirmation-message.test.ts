...
function buildConfirmationMessage(
  ...
  return {
    description: 'El fabricante ha recibido tu solicitud y se pondrá en contacto contigo pronto.',
    nextSteps: 'Mientras tanto, puedes revisar otras cotizaciones o crear una nueva.',
    timeline: 'Recibirás respuesta en 24-48 horas hábiles',
    title: `Cotización enviada el ${formattedDate}`,
    vendorContact: tenantConfig?.contactPhone ? `Contacto del fabricante: ${tenantConfig.contactPhone}` : undefined,
  };
}
...
describe('Confirmation Message - Content Validation (US3)', () => {
  ...
  describe('Message completeness', () => {
    ...
    it('should provide actionable next steps', () => {
      ...
      expect(message.nextSteps?.toLowerCase()).toMatch(/revisar|crear|ver|consultar/);
    });
  });
