...
describe("Contract: quote.submit", () => {
  ...
  it("should submit quote with valid contact information", async () => {
    ...
    const result = await testServer.quote["submit"](validInput);
  ...
  it("should validate input schema - invalid quoteId", async () => {
    ...
    await expect(async () => {
      await testServer.quote["submit"](invalidInput);
  ...
  it("should handle non-existent quote", async () => {
    ...
    await expect(async () => {
      await testServer.quote["submit"](input);
  ...
  it("should handle quote with no items", async () => {
    ...
    const result = await testServer.quote["submit"](input);
    expect(result.status).toBe("sent");
  });
  ...
  it("should accept various phone number formats", async () => {
    ...
    for (const phone of phoneFormats) {
      ...
      try {
        ...
        throw new Error(
          `Phone format "${phone}" failed: ${(error as Error).message}`
        );
      }
    }
  });
  ...
  it("should handle double submission gracefully", async () => {
    ...
    // First submission
    const firstResult = await testServer.quote["submit"](input);
    ...
    try {
      const secondResult = await testServer.quote["submit"](input);
      expect(secondResult.status).toBe("sent");
    } catch (error) {
      // If it fails, it should be a business logic error with Spanish message
      expect((error as Error).message).toMatch(/ya.*enviada|already.*sent/i);
    }
  });
