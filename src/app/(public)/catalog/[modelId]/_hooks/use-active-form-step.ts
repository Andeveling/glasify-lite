...
function createSectionObserver(
  ...
  return new IntersectionObserver((entries) => {
    for (const entry of entries) {
      const stepId = entry.target.getAttribute('data-step-id') as FormStepId | null
...
export function useActiveFormStep(
  ...
  useEffect(() => {
    ...
    const observer = createSectionObserver((stepId, ratio) => {
      ...
      const { step: mostVisibleStep, ratio: maxRatio } = findMostVisibleStep(intersectionRatios)
