...
export function GlassTypeCardSimple({ isSelected, option }: GlassTypeCardSimpleProps) {
  ...
  return (
    <motion.div animate={{ scale: isSelected ? SELECTED_SCALE : 1 }} transition={{ duration: 0.2 }}>
      <Label
        className={cn(
      ...
      >
        ...
        <motion.div
          animate={{
            backgroundColor: isSelected ? 'rgb(var(--primary-rgb))' : 'rgb(var(--muted-rgb))',
          }}
          className={cn(
        ...
        >
          <Icon
            className={cn('size-10 transition-colors duration-300', isSelected && 'text-primary')}
          />
        </motion.div>
        ...
        <div className="flex flex-1 items-center justify-between gap-3">
          ...
          <div className="flex-1 space-y-0.5">
            <motion.h4
              ...
              className={cn('font-semibold text-sm leading-tight transition-colors duration-300')}
              transition={{ duration: 0.2 }}
            >
              {option.name}
            </motion.h4>
            <p className="line-clamp-1 text-muted-foreground text-xs">{option.title}</p>
          </div>
