...
import { FORM_STEPS, type FormStepId, getStepProgress } from './form-steps-config'
...
export function ScrollProgressIndicator({
  ...
  const currentStepIndex = FORM_STEPS.findIndex((step) => step.id === activeStep) + 1
      ...
      <div className="px-4 py-3 sm:px-6">
          ...
          {FORM_STEPS.map((step, index) => {
            ...
            return (
              <div className="flex flex-1 flex-col items-center gap-1" key={step.id}>
                ...
                <motion.div
                  animate={{
                    opacity: isActive || isCompleted ? ACTIVE_STEP_OPACITY : INACTIVE_STEP_OPACITY,
        ...
        <div className="mt-3 flex items-center justify-between border-t pt-2">
          <div className="flex items-center gap-2">
            <Badge variant={activeStep === 'dimensions' ? 'default' : 'secondary'}>
          ...
          <span className="text-muted-foreground text-xs">{progressPercentage}% completado</span>
        </div>
      </div>
    </Card>
  )
