...
export function ColorChipOption({ index, isSelected, modelColor, useGrid }: ColorChipOptionProps) {
  const surcharge = modelColor.surchargePercentage

  return (
    <motion.div
      animate={isSelected ? { scale: 1.08, opacity: 1 } : { scale: 1, opacity: 0.7 }}
    ...
    >
      ...
      >
        <RadioGroupItem className="sr-only" id={modelColor.id} value={modelColor.color.id} />
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          {modelColor.color.ralCode && (
            <span className="text-muted-foreground text-xs">{modelColor.color.ralCode}</span>
          )}
