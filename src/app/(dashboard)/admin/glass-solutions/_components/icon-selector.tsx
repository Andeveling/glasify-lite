...
const GLASS_SOLUTION_ICONS: { name: string; icon: LucideIcon; category: string }[] = [
  ...
  { category: 'Temperatura', icon: ThermometerSnowflake, name: 'ThermometerSnowflake' },
...
export function IconSelector({ value, onChangeAction, disabled }: IconSelectorProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-4">
        <RadioGroup disabled={disabled} onValueChange={onChangeAction} value={value ?? undefined}>
            ...
            {filteredIcons.map(({ name, icon: IconComponent }) => (
              <div className="flex flex-col items-center gap-1" key={name}>
                ...
                >
                  ...
                  <RadioGroupItem className="sr-only" id={`icon-${name}`} value={name} />
                </Label>
      ...
      {value && (
        <div className="flex items-center gap-2 rounded-md border p-3">
          <span className="text-muted-foreground text-sm">Icono seleccionado:</span>
          <div className="flex items-center gap-2">
            {(() => {
              const selectedIcon = GLASS_SOLUTION_ICONS.find((icon) => icon.name === value);
              if (selectedIcon) {
