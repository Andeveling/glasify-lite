...
export function parsePattern(pattern: string): PanelDescriptor[] {
  if (!PATTERN_REGEX.test(pattern)) {
    throw new Error(`Invalid pattern "${pattern}". Only X (movable) and O (fixed) characters are allowed.`);
  }
...
export function isValidPattern(pattern: string): boolean {
  return PATTERN_REGEX.test(pattern) && pattern.length >= 1 && pattern.length <= 10 && hasAtLeastOneMovable(pattern);
}
