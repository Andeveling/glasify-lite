...
describe("Integration: Admin Panel Access Flow", () => {
  it("should handle glass model creation and management through admin endpoint", async () => {
    ...
    const createdModel = catalogResult.items.find(
      (model) => model.id === createResult.modelId
    );
  ...
  it("should validate admin model operations with dimension constraints", async () => {
    ...
    const constraintModel = catalogResult.items.find(
  ...
  it("should handle admin bulk model operations and data consistency", async () => {
    ...
    for (let i = 0; i < modelCount; i++) {
      const modelData = {
        accessoryPrice:
          i === 0 ? null : ACCESSORY_BASE_PRICE + i * ACCESSORY_INCREMENT,
    ...
    for (const modelId of modelIds) {
      const catalogModel = catalogResult.items.find(
    ...
    for (const testCase of testCases) {
      ...
      if (testCase.shouldFail) {
        ...
        if (
          ...
          "modelId" in operationResult
        ) {
          expect((operationResult as { modelId: string }).modelId).toMatch(
            VALID_CUID_REGEX
          );
        }
      }
