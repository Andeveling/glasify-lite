...
const mockUseQuery = api.catalog["get-model-by-id"].useQuery as ReturnType<
  typeof vi.fn
>;
...
describe("ModelFormPage", () => {
  ...
  it("should render model name and manufacturer", () => {
    ...
    expect(
      screen.getByRole("heading", { name: MODEL_NAME_REGEX })
  ...
  it("should render the parameterization form fields", () => {
    render(<ModelForm modelId="model-123" />);
    expect(screen.getByLabelText(WIDTH_LABEL_REGEX)).toBeInstanceOf(
      HTMLElement
    );
    expect(screen.getByLabelText(HEIGHT_LABEL_REGEX)).toBeInstanceOf(
      HTMLElement
    );
    expect(screen.getByLabelText(QUANTITY_LABEL_REGEX)).toBeInstanceOf(
      HTMLElement
    );
    expect(screen.getByLabelText(SERVICES_LABEL_REGEX)).toBeInstanceOf(
      HTMLElement
    );
    expect(
      screen.getByRole("button", { name: ADD_TO_QUOTE_BUTTON_REGEX })
    ).toBeInstanceOf(HTMLElement);
  });
