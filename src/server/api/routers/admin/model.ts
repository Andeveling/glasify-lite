 ...
 */
function buildOrderByClause(sortBy: string, sortOrder: 'asc' | 'desc'): Prisma.ModelOrderByWithRelationInput {
...
export const modelRouter = createTRPCRouter({
   ...
   */
  'add-cost-breakdown': adminProcedure.input(addCostBreakdownSchema).mutation(async ({ ctx, input }) => {
   ...
   */
  'delete-cost-breakdown': adminProcedure.input(deleteCostBreakdownSchema).mutation(async ({ ctx, input }) => {
  ...
  list: adminProcedure.input(listModelsSchema).query(async ({ ctx, input }) => {
    const { limit, page, search, sortBy, sortOrder, status, profileSupplierId } = input;
   ...
   */
  update: adminProcedure.input(updateModelSchema).mutation(async ({ ctx, input }) => {
    const { data, id } = input;
    ...
    if (data.compatibleGlassTypeIds && data.compatibleGlassTypeIds.length > 0) {
      const glassTypes = await ctx.db.glassType.findMany({
        select: {
          id: true,
          isActive: true,
    ...
    const priceChanged =
      ...
      (data.costPerMmWidth !== undefined &&
        data.costPerMmWidth.toString() !== currentModel.costPerMmWidth.toString()) ||
   ...
   */
  'update-cost-breakdown': adminProcedure.input(updateCostBreakdownSchema).mutation(async ({ ctx, input }) => {
    const { id, data } = input;
