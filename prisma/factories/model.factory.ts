 ...
 */
export type ModelCreateData = Omit<
 ...
 */
function validateModelBusinessLogic(
	validated: z.infer<typeof modelInputSchema>,
): ValidationError[] {
	const errors: ValidationError[] = [];

	// Validate width constraints (min < max)
	const widthError = validateMinMax(
		validated.minWidthMm,
	...
	if (validated.costPerMmWidth > MAX_COST_PER_MM) {
		errors.push({
			code: "COST_TOO_HIGH",
			context: {
				costPerMmWidth: validated.costPerMmWidth,
				max: MAX_COST_PER_MM,
			},
	...
	if (validated.costPerMmHeight > MAX_COST_PER_MM) {
		errors.push({
			code: "COST_TOO_HIGH",
			context: {
				costPerMmHeight: validated.costPerMmHeight,
				max: MAX_COST_PER_MM,
			},
	...
	if (
		validated.accessoryPrice !== undefined &&
		validated.accessoryPrice !== null
	) {
		const accessoryError = validatePrice(
			validated.accessoryPrice,
			"accessoryPrice",
	...
	const glassTypesError = validateNonEmpty(
	...
	if (
		...
		(validated.profitMarginPercentage < MIN_PROFIT_MARGIN ||
			validated.profitMarginPercentage > MAX_PROFIT_MARGIN)
	) {
		errors.push({
			code: "OUT_OF_RANGE",
			context: {
...
function toModelCreateData(
	...
	return {
		accessoryPrice:
			validated.accessoryPrice !== undefined
				? new Decimal(validated.accessoryPrice)
				: undefined,
		...
		profitMarginPercentage:
			validated.profitMarginPercentage !== undefined
				? new Decimal(validated.profitMarginPercentage)
				: undefined,
 ...
 */
export function getSuccessfulModels(
	results: FactoryResult<ModelCreateData>[],
): ModelCreateData[] {
	return results
		.filter(
			(
				result,
			): result is FactoryResult<ModelCreateData> & {
				success: true;
