...
5. Check domain language requirements (Spanish UI)
...
```typescript
// ✅ Good: Comprehensive error handling with Spanish UI messages
import { logger } from "~/lib/logger";

export async function calculateQuoteItem(input: CalculateItemInput) {
  try {
    const result = await priceCalculator.calculate(input);
    logger.info('Quote item calculated successfully', { itemId: result.id });
    return { success: true, data: result };
  } catch (error) {
    logger.error('Failed to calculate quote item', { error, input });
    return { 
      success: false, 
      error: 'No se pudo calcular el precio del ítem. Intente nuevamente.' 
    };
  }
}
...
try {
  return await calculatePrice(item);
} catch (e) {
  console.log(e);
}
```
