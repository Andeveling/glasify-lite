...
# Description: Replace all instances of "vidrio" with "cristal" and "vidrios"
#              with "cristales" throughout the project
# Usage: ./replace-vidrio-to-cristal.sh
...
BACKUP_DIR="${PROJECT_ROOT}/.backups/vidrio-to-cristal-$(date +%s)"
LOG_FILE="${PROJECT_ROOT}/.backups/replacement-log-$(date +%Y%m%d-%H%M%S).txt"

echo -e "${BLUE}════════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Script: Replace 'vidrio' with 'cristal'${NC}"
echo -e "${BLUE}════════════════════════════════════════════════════════════${NC}"
echo ""
...
echo -e "${BLUE}Searching for files containing 'vidrio'...${NC}"
...
# Filter files that contain vidrio/vidrios
FILTERED_FILES=""
while IFS= read -r file; do
  if [ -n "$file" ] && grep -q -E "vidrio|vidrios" "$file" 2>/dev/null; then
...
if [ -z "$FILES_WITH_VIDRIO" ]; then
  echo -e "${YELLOW}No files containing 'vidrio' or 'vidrios' found.${NC}"
  exit 0
fi

FILE_COUNT=$(echo "$FILES_WITH_VIDRIO" | wc -l)
echo -e "${GREEN}Found ${FILE_COUNT} files containing 'vidrio' or 'vidrios'${NC}"
echo ""
...
while IFS= read -r file; do
  ...
  BEFORE=$(grep -o -E "vidrio|vidrios" "$file" 2>/dev/null | wc -l)
  ...
  # Replace in the file
  # First replace "vidrios" (plural) - must be done before "vidrio"
  if ! sed -i 's/\bvidrios\b/cristales/g' "$file"; then
    echo -e "${RED}✗${NC} ${RELATIVE_PATH} (error in sed)"
    continue
  fi
  
  # Then replace "vidrio" (singular)
