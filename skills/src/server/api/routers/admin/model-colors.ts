...
export const modelColorsRouter = createTRPCRouter({
   ...
   */
  listByModel: adminProcedure.input(modelIdSchema).query(async ({ ctx, input }) => {
    try {
   ...
   */
  getAvailableColors: adminProcedure.input(modelIdSchema).query(async ({ ctx, input }) => {
    try {
   ...
   * Handles default color logic and tenant validation
   */
  assign: adminProcedure.input(modelColorAssignSchema).mutation(async ({ ctx, input }) => {
    try {
   ...
   */
  setDefault: adminProcedure.input(modelColorIdSchema).mutation(async ({ ctx, input }) => {
    try {
   ...
   */
  unassign: adminProcedure.input(modelColorIdSchema).mutation(async ({ ctx, input }) => {
    try {
      ...
      z.object({
        ...
        assignments: z.array(
          z.object({
            ...
            surchargePercentage: z.number().min(0).max(MAX_SURCHARGE_PERCENTAGE),
          }),
        ),
      }),
    )
